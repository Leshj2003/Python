...
if __name__ == "__main__":      #当程序调用执行时，执行第一个例子，程序入口
    #调用函数
    main()
...
#影片详情超链接的规则
findLink = re.compile(r'<a href="(.*?)">')     #创建正则表达式，表示规则,r忽略/号特殊符号等
#图片的链接
...
def getData(baseurl):
    ...
    # 2、逐一解析数据
        ...
        for item in soup.find_all('div',class_="item"):     #查找符合要求的字符串，形成列表
            ...
            #影片详情的超链接
            link = re.findall(findLink,item)[0]     #re库，用来查找指定的字符串，正则表达式[0]找到的第一个
            data.append(link)#添加链接

            imgSrc = re.findall(findImgSrc,item)[0]
            data.append(imgSrc)     #添加图片

            titles = re.findall(findTitle,item)     #片名可能只有中文，无外文
            if(len(titles)==2):
                ctitle = titles[0]
            ...
            datalist.append(data)       #处理好的信息放入datalist

    return datalist

#得到指定的一个url的网页内容
def askURL(url):
    #用户代理伪装，告诉服务器我们是什么类型的浏览器，我们可以接受什么水平的文件
    head = {        #模拟浏览器头部信息
